...
:::info
Pass the `--build` flag to the script to build images from source, or `--tag X.Y.Z` to use a tagged image. Pass `--seed` to seed Marquez with dummy metadata. Be advised that the lineage graph produced by this quickstart will differ as a result. 
:::
...
In this example, we show how you can collect dataset and job metadata using the [Marquez](https://marquezproject.github.io/marquez/) [LineageAPI](https://marquezproject.github.io/marquez/openapi.html#tag/Lineage) and [UI](https://github.com/MarquezProject/marquez/tree/main/web).

When you submit a lineage event, you first need to define a unique `run ID` similar to `d46e465b-d358-4d32-83d4-df660ff614dd`. UUID format is recommended, and it should be unique. This run ID will enable the tracking of run-level metadata over time for a _job_ that may have a name, like `my-job`. So, let's get started!
...
* Watch [Cross-platform Data Lineage with OpenLineage](https://www.youtube.com/watch?v=pLBVGIPuwEo).
* Watch [Observability for Data Pipelines with OpenLineage](https://2021.berlinbuzzwords.de/member/julien-le-dem).
* Watch [Data Observability and Pipelines: OpenLineage and Marquez](https://mattturck.com/datakin/).
* Watch [Data Lineage and Observability with Marquez and OpenLineage](https://bigdatatechwarsaw.eu/edition-2021/).
