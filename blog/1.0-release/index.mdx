...
This is why we’re seeing increasing demand for OpenLineage support, and we’re super excited to see more and more data engineers, developers and database managers joining our community. 

Momentum seems to be growing behind OpenLineage as the dominant standard for runtime lineage collection. At the same time, a consensus appears to be forming that an open spec will get us closest to 100% coverage, one that is highly granular and also general-purpose enough to be consistent across various data workloads.

### An Evolving Spec

Now that we’re at v1.0, static lineage has made its way to OpenLineage! Traditionally, OpenLineage has focused on “runtime” lineage - i.e., metadata generated when jobs are run. Capturing information as transformations of datasets occur enables precise descriptions of those transformations. 

The 1.0 release reflects the fact that demand has emerged for "design-time" lineage. The concept behind this is that even when datasets are not being touched, lineage metadata about them can still be useful and valuable.
...
Static lineage promises to fill the blind spots that dynamic lineage could not reach, offering a macroscopic view of how data flows and is accessed throughout an entire organization.
